...
from lc_tools import is_same_tree

def main():
    sol = Solution()
    root = TreeNode(1, left=TreeNode(3, right=TreeNode(2)))
    sol.recoverTree(root)
    assert is_same_tree(root, TreeNode(3, left=TreeNode(1, right=TreeNode(2)))), 'fails'

    root = TreeNode(3, left=TreeNode(1), right=TreeNode(4, left=TreeNode(2)))
    sol.recoverTree(root)
    assert is_same_tree(root, TreeNode(2, left=TreeNode(1), right=TreeNode(4, left=TreeNode(3)))), 'fails'

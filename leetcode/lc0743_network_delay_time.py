...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        mincost = {}  # latest mincost from start node to each node

        hq = [(0, k)]  # node, cost to this node, start at k
        heapq.heapify(hq)

        while hq:
            cost, cur = heapq.heappop(hq)
            if cur not in mincost or cost < mincost[cur]:  # already visited or found better cost
                mincost[cur] = cost
                for nxt, w in adj_list[cur]:
                    heapq.heappush(hq, (cost + w, nxt))  # explore and enque neighbor nodes

        if len(mincost) < n:  # cannot visit all nodes
            return -1
        else:

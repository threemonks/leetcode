...
class Solution:
    def countRangeSum(self, nums: List[int], lower: int, upper: int) -> int:
        ...
        for psum in presum:
            """
            ind = bisect.bisect_right(sorted_presum, psum - lower) will get the smallest ind that sorted_presum[ind] > psum - lower. 
            So all sums from index 0 to ind-1 inclusive will be <= psum - lower. The count for those sums would be (ind - 1 + 1). 
            The +1 here is because the index of BIT starts from 1.
            Same for bisect.bisect_left(sorted_presum, sum_j - upper).
            """

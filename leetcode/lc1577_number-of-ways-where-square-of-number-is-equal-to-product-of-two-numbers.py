...
Note: once we know occurance count of each number, we can get the number of triplets a three number can provide
    note if j<k but nums2[j] == nums2[k], i.e., square in nums1, but same number appears in nums2 more than once
    so contributing triplets count
    if nums2[j] != nums2[k]: ans += cnt2[nums2[j]]*cnt2[nums2[k]]
    else: ans += cnt2[nums2[j]]*(cnt2[nums2[j]]-1)

mistakes:
1. should use counter, separately consider whether val at j and k are same or not, calculate total triplets count
   instead of adding each permutation and count the permutations => TLE
"""

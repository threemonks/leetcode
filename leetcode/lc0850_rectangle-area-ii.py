...
if we can sort active events, then we can check active events list to find out the total active x length we got on this specific y value, multiple this by the difference of this y value and next, we got area covered between these two y values
...
class Solution:
    def rectangleArea(self, rectangles: List[List[int]]) -> int:
        ...
        def calc_area():
            ...
            for x1, x2 in active:
                covered += max(0, x2-max(cur, x1)) # cur could be bigger than x2, i.e., a shorter interval start at the same time comes later, but ends sooner
                cur = max(cur, x2)

...
mistakes1:
1. we reset a to headB when a is null, not when a.next is null, i.e.,
    a = a.next if a else headB, instead of a = a.next if a.next else headB
   because when two lists have no intersection, the while loop will exit only if both a and b come to value None, so we need to assign a to headB when a is None, not when a.next is None
   trying to assign a to headB when a.next is None (a is not None yet), then a never become null, nor will b, so they will not become the same, thus the while loop never exits
